...
libcaesar.so: caesar.o
	$(CC) $(LDFLAGS) -o libcaesar.so caesar.o # флаг -o означает имя выходного объекта
...
install: libcaesar.so
	sudo cp libcaesar.so /usr/local/lib/ # cp копирует файл библиотеки в системную директорию, чтобы библиотека стала доступна
	sudo ldconfig # ldconfig обновляет линкер, чтобы он обнаружил скопированный файл

test: libcaesar.so input.txt
	python3 test.py ./libcaesar.so K input.txt output.txt # Запускаем питон скрипт с аргументами: путь к .so, ключ 'K', input/output файлы

clean:
  rm -f *.o *.so output.txt decrypted.txt # удаляет все .o и .so и output.txt и 
